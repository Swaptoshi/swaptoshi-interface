...
const Navbar = ({ searchOptions, handleCart }) => {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      if (
        centerSearchRef.current &&
        !centerSearchRef.current.contains(event.target)
      ) {
        setSearchOpen(false);
      }
      ...
      if (
        ...
        (!rightSearchInsideRef.current ||
          !rightSearchInsideRef.current.contains(event.target))
      ) {
