export const slidersData = [
  {
    id: 1,
    coverImage: '/assets/cover-images/img-2.png',
    image: 'assets/images/degodsicon.png',
    title: 'DeGods',
    sliderContent: [
      {
        icons: ['/assets/images/logo/unicorn-logo.png', '/assets/images/opensea-grey.png'],
        names: ['Uniswap', 'OpenSea', 'X2Y2', 'LooksRare'],
        ethFloor: [29.09, 29.09, 83.14, 63.39],
        listings: [206, 199, 1, 5],
      },
    ],
  },
  {
    id: 2,
    coverImage: '/assets/cover-images/img-1.png',
    image: '/assets/cover-images/img-1.png',
    title: 'Bored Ape Yacht Club',

    sliderContent: [
      {
        icons: ['/assets/images/logo/unicorn-logo.png', '/assets/images/opensea-grey.png'],
        names: ['Uniswap', 'OpenSea', 'X2Y2', 'LooksRare'],
        ethFloor: [29.09, 29.09, 83.14, 63.39],
        listings: [206, 199, 1, 5],
      },
    ],
  },
];

// Data in ETH
export const allTableDataETH = [
  {
    id: 1,
    title: 'DeGods',
    image: 'https://i.seadn.io/gcs/files/c6cb0b1d6f2ab61c0efacf00e62e2230.jpg?w=500&auto=format',
    coverImage: 'https://i.seadn.io/gcs/files/0f98e562496514deec72096435a77eef.jpg?w=1133',
    desc: ' The DeGods is a collection of 10,000 unique Bored Ape NFTs— unique digital collectibles living on the Ethereum blockchain.',
    floor: 4.7,
    floorChange: 6,
    volume: 3376,
    volumeChange: 70,
    items: 8.9,
    owners: 2.5,
    // tab: "1D"
  },
  {
    id: 2,
    title: 'Bored Ape Yacht Club',
    image: '/assets/images/bored.png',
    coverImage:
			'https://i.seadn.io/gae/i5dYZRkVCUK97bfprQ3WXyrT9BnLSZtVKGJlKQ919uaUB0sxbngVCioaiyu9r6snqfi2aaTyIvv6DHm4m2R3y7hMajbsv14pSZK8mhs?w=1517',
    desc: ' The Bored Ape Yacht Club is a collection of 10,000 unique Bored Ape NFTs— unique digital collectibles living on the Ethereum blockchain.',
    floor: 26.5,
    floorChange: 9,
    volume: 2165,
    volumeChange: 60,
    items: 10,
    owners: 5.6,
    // tab: "1W"
  },
  {
    id: 3,
    title: 'Mutant Ape Yacht Club',
    image: '/assets/images/mount.png',
    coverImage:
			'https://i.seadn.io/gae/5c-HcdLMinTg3LvEwXYZYC-u5nN22Pn5ivTPYA4pVEsWJHU1rCobhUlHSFjZgCHPGSmcGMQGCrDCQU8BfSfygmL7Uol9MRQZt6-gqA?w=1133',
    desc: ' The Mutant Ape Yacht Club is a collection of 10,000 unique Bored Ape NFTs— unique digital collectibles living on the Ethereum blockchain.',
    floor: 4.65,
    floorChange: 10,
    volume: 891,
    volumeChange: 17,
    items: 19.5,
    owners: 11.4,
    // tab: "1W"
  },
];

// Data in USD
export const allTableDataUSD = [
  {
    id: 1,
    title: 'DeGods',
    coverImage: 'https://i.seadn.io/gcs/files/0f98e562496514deec72096435a77eef.jpg?w=1133',
    image: 'https://i.seadn.io/gcs/files/c6cb0b1d6f2ab61c0efacf00e62e2230.jpg?w=500&auto=format',
    floor: 7835,
    desc: ' The DeGods is a collection of 10,000 unique Bored Ape NFTs— unique digital collectibles living on the Ethereum blockchain.',
    floorChange: 6,
    volume: 5636917,
    volumeChange: 50,
    items: 8.9,
    owners: 2.5,
  },
  {
    id: 2,
    title: 'Bored Ape Yacht Club',
    image: '/assets/images/bored.png',
    coverImage:
			'https://i.seadn.io/gae/i5dYZRkVCUK97bfprQ3WXyrT9BnLSZtVKGJlKQ919uaUB0sxbngVCioaiyu9r6snqfi2aaTyIvv6DHm4m2R3y7hMajbsv14pSZK8mhs?w=1517',
    desc: ' The Bored Ape Yacht Club is a collection of 10,000 unique Bored Ape NFTs— unique digital collectibles living on the Ethereum blockchain.',
    floor: 44280,
    floorChange: 9,
    volume: 3617056,
    volumeChange: 68,
    items: 10,
    owners: 5.2,
  },
  {
    id: 3,
    title: 'Mutant Ape Yacht Club',
    image: '/assets/images/mount.png',
    coverImage:
			'https://i.seadn.io/gae/5c-HcdLMinTg3LvEwXYZYC-u5nN22Pn5ivTPYA4pVEsWJHU1rCobhUlHSFjZgCHPGSmcGMQGCrDCQU8BfSfygmL7Uol9MRQZt6-gqA?w=1133',
    desc: ' The Mutant Ape Yacht Club is a collection of 10,000 unique Bored Ape NFTs— unique digital collectibles living on the Ethereum blockchain.',
    floor: 7280,
    floorChange: 8,
    volume: 1417056,
    volumeChange: 17,
    items: 19.5,
    owners: 11.4,
  },
];

// Data in ETH
// export const allTableDataETH = [
//     {
//         id: 1,
//         title: "DeGods",
//         image: "https://i.seadn.io/gcs/files/c6cb0b1d6f2ab61c0efacf00e62e2230.jpg?w=500&auto=format",
//         floor: 4.7,
//         floorChange: 6,
//         volume: 3376,
//         volumeChange: 70,
//         items: 8.9,
//         owners: 2.5,
//         // tab: "1D"
//     },
//     {
//         id: 2,
//         title: "Bored Ape Yacht Club",
//         image: "/assets/images/bored.png",
//         floor: 26.5,
//         floorChange: 9,
//         volume: 2165,
//         volumeChange: 60,
//         items: 10,
//         owners: 5.6,
//         // tab: "1W"
//     },
//     {
//         id: 3,
//         title: "Mutant Ape Yacht Club",
//         image: "/assets/images/mount.png",
//         floor: 4.65,
//         floorChange: 8,
//         volume: 891,
//         volumeChange: 17,
//         items: 19.5,
//         owners: 11.4,
//         // tab: "1W"
//     },

// ]

// // Data in USD
// export const allTableDataUSD = [
//     {
//         id: 1,
//         title: "DeGods",
//         image: "https://i.seadn.io/gcs/files/c6cb0b1d6f2ab61c0efacf00e62e2230.jpg?w=500&auto=format",
//         floor: 7835,
//         floorChange: 6,
//         volume: 5636917,
//         volumeChange: 50,
//         items: 8.9,
//         owners: 2.5,
//     },
//     {
//         id: 2,
//         title: "Bored Ape Yacht Club",
//         image: "/assets/images/bored.png",
//         floor: 44280,
//         floorChange: 9,
//         volume: 3617056,
//         volumeChange: 68,
//         items: 10,
//         owners: 5.2,
//     },
//     {
//         id: 3,
//         title: "Mutant Ape Yacht Club",
//         image: "/assets/images/mount.png",
//         floor: 7280,
//         floorChange: 8,
//         volume: 1417056,
//         volumeChange: 17,
//         items: 19.5,
//         owners: 11.4,
//     },
// ]
